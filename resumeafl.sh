...
if [ -z "$1" ]; then
	...
	afl-fuzz -U -m none -i - -o afl_outputs -t 1000 -M master -- python harness.py @@ || exit 1
else 
	echo "[*] Unicorefuzz running AFL worker with id $1"
	afl-fuzz -U -m none -i - -o afl_outputs -t 1000 -S fuzzer$1 -- python harness.py @@ || exit 1
fi
